<template>
  <div>
    ...
    <!-- 病因编写对话框 -->
    <el-dialog title="Problem" :visible.sync="reasonFormVisible">
      <el-input
        type="textarea"
        :rows="8"
        placeholder="content"
...
export default {
  ...
  methods: {
    ...
    dataPackage() {
      ...
      oCheck += " Check_total_price" + this.checkTotalPrice + "元 ";
      oDrug += " Drug_total_price" + this.drugTotalPrice + "元 ";
    ...
    addCheck(chId) {
      request
        ...
        .then((res) => {
          if (res.data.status != 200) return this.$message.error("Fail");
          //后端传过来的是对象，表格绑定的数组
    ...
    requestCheck() {
      request
        ...
        .then((res) => {
          toLoad();
          if (res.data.status != 200) this.$message.error("Fail");
